...
class JsonInputReader(BaseInputReader):
    ...
    def _parse_dataset(self, dataset_path, dataset):
        ...
        for document in tqdm(documents, desc="Parse dataset '%s'" % dataset.label):
            ...
            self._parse_document_multi_task(document, dataset)
            # break
    ...
    def _parse_document(self, doc, dataset) -> Document:
        ...
        # print("sci_relations", sci_relations)
        # print("sem_relations", sem_relations)
        ...
        # print(len(entities[0].tokens))
        # print(entities[0].entity_type.short_name)
        # print("***************")

        # parse relations
        # try:
        relations = self._parse_relations(jrelations, entities, dataset)
            # print(relations[0].relation_type.short_name)
            # print(relations[0].as_tuple())
        # except:
        #     print("doc", doc['orig_id'])
        #     print("tokens", jtokens)
        #     print("jentities", jentities)
    ...
    def _parse_document_multi_task(self, doc, dataset) -> Document:
        jtokens = doc['tokens']
        jrelations = doc['relations']
        jentities = doc['entities']

        sci_jentities = doc['sci_entities']
        sci_jrelations = doc['sci_relations']

        sem_jentities = doc['sem_entities']
        sem_jrelations = doc['sem_relations']

        # print("sci_entities", sci_jentities)
        # print("sci_relations", sci_jrelations)
        # print("sem_entities", sem_jentities)
        # print("sem_relations", sem_jrelations)
        # print("*"*100)
        ...
        # parse entity mentions
        sci_entities = self._parse_entities(sci_jentities, doc_tokens, dataset)
        sem_entities = self._parse_entities(sem_jentities, doc_tokens, dataset)
        # print(entities[0].phrase)
        # print(len(entities[0].tokens))
        # print(entities[0].entity_type.short_name)
        # print("***************")
        ...
        # parse relations
        # try:
        sci_relations = self._parse_relations(sci_jrelations, sci_entities, dataset)
        sem_relations = self._parse_relations(sem_jrelations, sem_entities, dataset)
            # print(relations[0].relation_type.short_name)
            # print(relations[0].as_tuple())
        # except:
        #     print("doc", doc['orig_id'])
        ...
        # create document
        document = dataset.create_document_multi_task(doc_tokens, sci_entities, sci_relations, sem_entities, sem_relations, doc_encoding)
    ...
    def _parse_entities(self, jentities, doc_tokens, dataset) -> List[Entity]:
        ...
        for entity_idx, jentity in enumerate(jentities):
            # print(self._entity_types)
            # print("jentity['type']", jentity)
